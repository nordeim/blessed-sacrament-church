...
describe("content", () => {
  it("lifeTimeline has 7 entries each with year/title/description length>20", () => {
  ...
  it("grounds has 3 sections each with id in allowlist, image/imageFallback/imageAlt non-empty", () => {
    ...
    expect(grounds.map((g) => g.id)).toEqual(["main-church", "damien-hall", "garden"]);
  ...
  it("givingOptions has 6 entries each title/description length>0 and icons include globe", () => {
