...
- `test` in `vite.config.ts` — `{ globals: true, environment: "jsdom", setupFiles: ["src/test/setup.ts"], include: ["src/**/*.{test,spec}.{ts,tsx}"], exclude: ["e2e/**", "node_modules/**", "playwright-report/**", "test-results/**"] }` — keeps `e2e/**` out of unit runs. **Note:** `src/test/setup.ts` is **restored** (F1) — `src/` totals 40 source files + 17 test files + 1 setup, and `pnpm test` reports 17 files / 98 tests green (BSC suite + `docs-contract`/`repo-hygiene`/`ci-workflow` guards).
...
- Palette: `bsc-cream / parchment(+dark) / stone / ink / charcoal / sapphire-{50,100,200,300,400,500,600,700,800,900,950} / gold-{100,200,300,400,500,600,700} / terracotta-{400,500,600} / pine-{500,600}` plus `shadow-bsc/bsc-lg` (26 colors + 2 shadows). Use semantic names (`bsc-sapphire-600`) not hex. `terracotta-600` (#8f5038) is the AA text-bearing step — chip labels and other 10–14 px text on parchment must compute ≥ 4.5:1.
...
- `Footer` (`components/Footer.tsx`): 4-col (`Explore` + `Get involved` from `footerNav` split + parish/visit/contact blocks), `divider-weave-thin`, and consumes `site.ts` + `nav.ts`. Copy is BSC-specific (Tent of Meeting, SS.CC, Damien of Molokai, Corpus Christi, five tongues). Social icons from `site.ts` (Facebook/Instagram via custom SVG in `SocialIcons`) + `archdiocese` + `mapsUrl`/`mapsEmbedSrc` (1 Commonwealth Drive); address `1 Commonwealth Drive`; Reception/office hours `Mon–Fri 9–17.30`; phone `+65 6474 0582` / WhatsApp `+65 9170 9133`; copyright. Tagline: site.tagline "A Household of Faith, Hope & Love." / vision.
...
| `pnpm test` / `npm run test` | Vitest `jsdom` — `vitest run` | ✅ **17 files / 98 tests green** | Harness restored (F1): `src/test/setup.ts` + BSC-adapted suite (cn / nav / content / site / massDay / monogram / deepLinks / Button / Accordion / SafeImage / Header / BackToTop / ScrollProgress / Layout / useScrollProgress / useScrollSpy) + round-13 guards (`ci-workflow` / `repo-hygiene` / `docs-contract`). |
...
| `pnpm test:e2e` / `npm run test:e2e` | Playwright `chromium` — `playwright test` (8 spec files + helpers — 51 tests) | ✅ **51/51 green** | Retargeted to BSC copy (F2B: `1 Commonwealth Drive`, `A tent of meeting`, `Queenstown EW19 + Commonwealth EW20`, `Corpus Christi`). |
...
pnpm test               # vitest run — 17 files / 98 tests green
...
- Use existing `bsc-*` tokens before introducing new colors. Tokens 26 colors + 2 shadows are the budget — frame the Tent of Meeting / Damien Centre / Wihardja Stations imagery with them, don't add arbitrary `bg-[#...]`.
...
```
src/ (40 source + 17 test + 1 setup files — harness restored, 17 files / 98 tests green)
  ...
  index.css              # Tailwind v4 @theme (26 colors + 2 shadows) + @layer base/utilities (28 utilities: text-balance, bg-adobe-texture, bg-gold-bloom, bg-grain, divider-weave, divider-weave-thin, gold-rule, gold-rule-left, hero-ken-burns, img-zoom, mask-fade-b, reveal, reveal-visible, rise-in + rise-in-d1..d4, menu-in, drawer-in, drawer-item-in, page-in, dot-pulse, card-lift, card-tint, link-underline, skip-link, bloom-drift + 9 keyframes gold-rule-draw/hero-ken-burns/rise-in/menu-in/drawer-in/drawer-item-in/page-in/halo-pulse/bloom-drift + themed scrollbar (sapphire thumb on parchment track, webkit + scrollbar-color))
  components/
...
public/
  images/ (9)            # hero-church.jpg, community.jpg, damien-hall.jpg, faith-formation.jpg, family-life.jpg, garden.jpg, liturgical.jpg, pastoral-care.jpg, youth.jpg (Vite publicDir → dist/images/ — upload alongside dist/index.html); all images local; + favicon.svg (referenced by index.html) + robots.txt (round-13)
...
- Tokens: see `src/index.css` `@theme`. Additions require design rationale in PR description. Tokens 26 colors + 2 shadows: `bsc-cream/parchment/parchment-dark/stone/ink/charcoal`, `sapphire-50..950` (11), `gold-100..700` (7), `terracotta-400/500/600` (3), `pine-500/600` (2) + `shadow-bsc/bsc-lg`. Only the imagery/content they frame is Queenstown's Tent of Meeting (folded blue roof, Damien Hall/Centre, Adoration Chapel, Wihardja Stations) — keep tokens stable.
...
- **Extending `@theme` with one-off hex values** — add a named `bsc-*` token or reuse an existing one. Tokens 26+2 shadows are the budget.
...
- All five gates are green: `pnpm lint` (0) + `pnpm typecheck` (0) + `pnpm test` (17 files / 98 tests) + `pnpm test:e2e` (51/51, BSC) + `pnpm build` (392.96kB); sixth check `pnpm test:e2e:built` 51/51 (built artifact + live host).
...
- Keep `README.md` + `AGENTS.md` + this file + `blessed-sacrament-church_SKILL.md` (or `risen-christ_SKILL.md` as historical redirect) in sync on version, routing, and data shape after every port/validation change. BSC package is `blessed-sacrament-church` 1.4.4; do not carry Risen Christ package name forward.
...
| 5 | Testing Strategy (17 files / 98 tests green + 51 E2E green BSC; porting checklist completed — historical) | Yes | ✅ |
...
| 10 | Project-Specific Standards (Architecture 40 source + 17 test + 1 setup tree + Data ownership 8 interfaces/7 images all local + Routing 17/7/9 + File Org + Design System 26+2) | Yes | ✅ |
