...
| `pnpm test` / `npm run test` | Vitest `jsdom` — `vitest run` | ✅ **21 files / 112 tests green** | Harness restored (F1): `src/test/setup.ts` + BSC-adapted suite (cn / nav / content / site / massDay / monogram / deepLinks / Button / Accordion / SafeImage / ScrollProgress / Layout / useScrollProgress / useScrollSpy) + guards (`ci-workflow` / `repo-hygiene` / `docs-contract` / `public-contract` / `token-integrity`) + round-16 suites (`Header` / `Layout.anchor`). |
...
pnpm test               # vitest run — 21 files / 112 tests green
...
- All five gates are green: `pnpm lint` (0) + `pnpm typecheck` (0) + `pnpm test` (21 files / 112 tests) + `pnpm test:e2e` (51/51, BSC) + `pnpm build` (466.20kB); sixth check `pnpm test:e2e:built` 51/51 (built artifact).
...
- Keep `README.md` + `AGENTS.md` + this file + `blessed-sacrament-queenstown_SKILL.md` in sync on version, routing, and data shape after every port/validation change. BSC package is `blessed-sacrament-church` 1.4.4; do not carry Risen Christ package name forward.
